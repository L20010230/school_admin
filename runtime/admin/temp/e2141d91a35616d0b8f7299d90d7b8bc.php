<?php /*a:2:{s:94:"/www/wwwroot/senior.test.jiayuanxueyuan.cn/app/admin/controller/Sys/view/action/api_index.html";i:1586854930;s:80:"/www/wwwroot/senior.test.jiayuanxueyuan.cn/app/admin/view/common/_container.html";i:1602393978;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script>
	...
	CodeGoods.initColumn = function () {
 		return [
			...
			{title: '操作名称', field: 'name', visible: true, align: 'left', valign: 'middle'},
 			...
 			{title: 'controller方法生成', field: 'is_controller_create', visible: true, align: 'center', valign: 'middle',formatter: 'CodeGoods.controllerFormatter'},
			{title: 'service方法生成', field: 'is_service_create', visible: true, align: 'center', valign: 'middle',formatter: 'CodeGoods.serviceFormatter'},
			{title: '日志生成', field: 'log_status', visible: false, align: 'center', valign: 'middle',formatter: 'CodeGoods.logFormatter'},
			{title: 'token认证', field: 'api_auth', visible: true, align: 'center', valign: 'middle',formatter: 'CodeGoods.tokenFormatter'},
			{title: '短信认证', field: 'sms_auth', visible: true, align: 'center', valign: 'middle',formatter: 'CodeGoods.smsFormatter'},
			{title: '图片验证码认证', field: 'captcha_auth', visible: true, align: 'center', valign: 'middle',formatter: 'CodeGoods.verifyFormatter'},
			{title: '缓存时间', field: 'cache_time', visible: true, align: 'center', valign: 'middle',formatter: 'CodeGoods.cacheFormatter'},
			{title: '请求方式', field: 'request_type', visible: true, align: 'center', valign: 'middle'},
			{title: '排序', field: 'sortid', visible: false, align: 'center', valign: 'middle',formatter: 'CodeGoods.sortFormatter'},
	...
	CodeGoods.cacheFormatter = function(value,row,index) {
		if(value){
			return value + 's';
		}	
	}
	...
	CodeGoods.tokenFormatter = function(value,row,index) {
		if(value == 1){
			return '<input class="mui-switch mui-switch-animbg api_auth'+row.id+'" type="checkbox" onclick="CodeGoods.updatestatus('+row.id+',0,\'api_auth\')" checked>';
		}else{
			return '<input class="mui-switch mui-switch-animbg api_auth'+row.id+'"  type="checkbox" onclick="CodeGoods.updatestatus('+row.id+',1,\'api_auth\')">';
		}
	}
	
	CodeGoods.smsFormatter = function(value,row,index) {
		if(value == 1){
			return '<input class="mui-switch mui-switch-animbg sms_auth'+row.id+'" type="checkbox" onclick="CodeGoods.updatestatus('+row.id+',0,\'sms_auth\')" checked>';
		}else{
			return '<input class="mui-switch mui-switch-animbg sms_auth'+row.id+'"  type="checkbox" onclick="CodeGoods.updatestatus('+row.id+',1,\'sms_auth\')">';
		}
	}
	
	CodeGoods.verifyFormatter = function(value,row,index) {
		if(value == 1){
			return '<input class="mui-switch mui-switch-animbg captcha_auth'+row.id+'" type="checkbox" onclick="CodeGoods.updatestatus('+row.id+',0,\'captcha_auth\')" checked>';
		}else{
			return '<input class="mui-switch mui-switch-animbg captcha_auth'+row.id+'"  type="checkbox" onclick="CodeGoods.updatestatus('+row.id+',1,\'captcha_auth\')">';
		}
	}
	...
	CodeGoods.sortFormatter = function(value,row,index) {
		return '<input type="text" value="'+value+'" onblur="CodeGoods.upsort('+row.id+',this.value)" style="width:40px; border:1px solid #ddd; text-align:center">';
	}
