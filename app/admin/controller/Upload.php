...
class Upload extends Admin{
	...
	private function upload($filekey){
		...
		$file_type = upload_replace(config('base.file_type')); //上传黑名单过滤
		if(!Validate::fileExt($file,$file_type) || !Validate::fileSize($file,config('base.file_size') * 1024 * 1024)){
	...
	private function thumb($imagesUrl,$upload_config_id){
		...
		if($configInfo){ 
			...
			//生成水印
			if(file_exists('.'.config('base.water_logo')) && config('base.water_status') && config('base.water_position')){
				$image->water('.'.config('base.water_logo'),config('base.water_position'))->save($targetimages); 
			}	
		}
	}
