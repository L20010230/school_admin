<?php /*a:2:{s:75:"/www/wwwroot/senior.test.jiayuanxueyuan.cn/app/admin/view/member/index.html";i:1669691416;s:80:"/www/wwwroot/senior.test.jiayuanxueyuan.cn/app/admin/view/common/_container.html";i:1602393978;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
			...
			<div class="ibox-content"> 
				<div class="row row-lg"> 
					<div class="col-sm-12"> 
						<div class="row" id="searchGroup">
							<div class="col-sm-2">
								<div class="input-group">
									...
									<input type="text" autocomplete="off" class="form-control" id="name_s" placeholder="姓名" />
								</div>
							</div>
							<div class="col-sm-2">
								<div class="input-group">
									...
									<input type="text" autocomplete="off" class="form-control" id="user_name" placeholder="账号" />
								</div>
							</div>
							<div class="col-sm-2">
								<div class="input-group">
							...
							<div class="col-sm-2">
								<div class="input-group">
									...
									<select class="form-control" id="role">
										...
										<option value="<?php echo $sql['member_role_id']; ?>"><?php echo $sql['role_name']; ?></option>
										<?php endforeach?>
									</select>
								</div>
							</div>
						...
						<?php if(in_array('admin/Member/add',session('admin.nodes')) || session('admin.role_id') == 1): ?>
						<button type="button" id="add" class="btn btn-primary button-margin" onclick="CodeGoods.add()">
						<i class="fa fa-plus"></i>&nbsp;添加
						</button>
						<?php endif; if(in_array('admin/Member/update',session('admin.nodes')) || session('admin.role_id') == 1): ?>
						<button type="button" id="update" class="btn btn-success button-margin" onclick="CodeGoods.update()">
						<i class="fa fa-pencil"></i>&nbsp;修改
						</button>
						<?php endif; if(in_array('admin/Member/delete',session('admin.nodes')) || session('admin.role_id') == 1): ?>
						<button type="button" id="delete" class="btn btn-danger button-margin" onclick="CodeGoods.delete()">
						<i class="fa fa-trash"></i>&nbsp;删除
						</button>
						<?php endif; if(in_array('admin/Member/view',session('admin.nodes')) || session('admin.role_id') == 1): ?>
						<button type="button" id="view" class="btn btn-info button-margin" onclick="CodeGoods.view()">
						<i class="fa fa-eye"></i>&nbsp;查看详情
						</button>
						<?php endif; if(in_array('admin/Member/change_password',session('admin.nodes')) || session('admin.role_id') == 1): ?>
...
<script>
	...
	CodeGoods.initColumn = function () {
 		return [
 			...
 			{title: '账号', field: 'user_name', visible: true, align: 'center', valign: 'middle',sortable: true},
	...
	CodeGoods.buttonFormatter = function(value,row,index) {
		if(row.member_id){
			var str= '';
			<?php if(in_array('admin/Member/update',session('admin.nodes')) || session('admin.role_id') == 1): ?>
			...
			<?php endif; if(in_array('admin/Member/change_password',session('admin.nodes')) || session('admin.role_id') == 1): ?>
			str += '<button type="button" class="btn btn-primary btn-xs" title="修改密码"  onclick="CodeGoods.change_password('+row.member_id+')"><i class="fa fa-lock"></i>&nbsp;修改密码</button>&nbsp;';
			<?php endif; ?>
			return str;
		}
	}
	...
	CodeGoods.add = function (value) {
		...
		var index = layer.open({type: 2,title: '添加',area: ['800px', '500px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Member/add'+url});
	...
	CodeGoods.update = function (value) {
		if(value){
			var index = layer.open({type: 2,title: '修改',area: ['800px', '500px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Member/update?member_id='+value});
			if(!IsPC()){layer.full(index)}
		}else{
			if (this.check()) {
				var idx = '';
				...
				var index = layer.open({type: 2,title: '修改',area: ['800px', '500px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Member/update?member_id='+idx});
				this.layerIndex = index;
				if(!IsPC()){layer.full(index)}
			}
		}
	...
	CodeGoods.view = function (value) {
		if(value){
			var index = layer.open({type: 2,title: '查看详情',area: ['800px', '500px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Member/view?member_id='+value});
			if(!IsPC()){layer.full(index)}
		}else{
			if (this.check()) {
				var idx = '';
				...
				var index = layer.open({type: 2,title: '查看详情',area: ['800px', '500px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Member/view?member_id='+idx});
				this.layerIndex = index;
				if(!IsPC()){layer.full(index)}
			}
		}
	...
	CodeGoods.change_password = function (value) {
		if(value){
			var index = layer.open({type: 2,title: '修改密码',area: ['600px', '350px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Member/change_password?member_id='+value});
			this.layerIndex = index;
			if(!IsPC()){layer.full(index)}
		}else{
			if (this.check()) {
				...
				idx = idx.substr(1);
				var index = layer.open({type: 2,title: '修改密码',area: ['600px', '350px'],fix: false, maxmin: true,content: Feng.ctxPath + '/Member/change_password?member_id='+idx});
				this.layerIndex = index;
				if(!IsPC()){layer.full(index)}
			}
		}
