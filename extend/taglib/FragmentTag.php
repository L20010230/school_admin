...
class FragmentTag extends TagLib
{
    ...
    protected $tags = array(
		'sql'    	 => ['attr' => 'query,connect', 'alias' => 'iterate'],
    ...
    {   
		...
		$connect.=($tag['connect'])?$tag['connect']:config('database.default');
		$tag['query'] = str_replace('pre_',config('database.connections.'.$connect.'.prefix'),$tag['query']);
		if(false == strpos($tag['query'],'.')){
			$tag['query'] = str_replace('\'','"',$tag['query']);
		}
        $data ="\\think\\facade\\Db::connect('".$connect."')->query('{$tag['query']}')";
