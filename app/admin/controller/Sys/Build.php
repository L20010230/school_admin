				...
				case (in_array($val['type'],[1,32])):
					...
					if($val['is_controller_create'] !== 0){
						...
						if(!empty($val['sql_query'])){
							...
							if($menuInfo['connect']){
								$str .="			\$res = \base\CommonService::loadList(\$sql,formatWhere(\$where),\$limit,\$orderby,'".$menuInfo['connect']."');\n";
							}else{
								$str .="			\$res = \base\CommonService::loadList(\$sql,formatWhere(\$where),\$limit,\$orderby);\n";
							}
					...
					if($val['is_controller_create'] !== 0){
						...
						if(empty($val['relate_table']) && empty($val['relate_field']) && empty($val['sql_query'])){
							...
							if(!empty($val['sql_query'])){
								...
								if($menuInfo['connect']){
									$str .= "			\$res = \base\CommonService::loadList(\$sql,formatWhere(\$where),config('my.max_dump_data'),\$orderby='','".$menuInfo['connect']."');\n";
								}else{
									$str .= "			\$res = \base\CommonService::loadList(\$sql,formatWhere(\$where),config('my.max_dump_data'),\$orderby='');\n";
								}
